...
results <- results %>% 
  filter(tourney_level %in% tours, surface == "Hard")
...
sr_current <- sr_df %>% 
  filter(Lag < 30, Rating >= 1600, Games >= 30)

#view(head(sr_current,30))

write_csv(sr_current, "SR_ratings_hard_jan21.csv")
...
ggplot(temp,
  ...
  labs(title = "Hard Court Player Ratings Through Time - Current Top 20 Rated Players (Stephenson ELO Variant)", 
       x = "Time", y = "Rating") +
