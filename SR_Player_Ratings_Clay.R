...
results <- results %>% 
  filter(tourney_level %in% tours, surface == "Clay")
...
sr_current <- sr_df %>% 
  filter(Lag <= 20, Rating >= 1600, Games >= 10)

head(sr_current)

#view(head(sr_current,30))

write_csv(sr_current, "SR_ratings_clay_jan21.csv")
...
sr_timeline_current <- sr_timeline %>% 
  ...
  select(Player, last_col(offset = 50):last_col())
...
ggplot(temp,
  ...
  labs(title = "Clay Court Player Ratings Through Time - Current Top 20 Rated Players (Stephenson ELO Variant)", 
       x = "Time", y = "Rating") +
